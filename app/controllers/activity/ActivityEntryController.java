...
public class ActivityEntryController extends AppController {
	
    public Result create() {           //  create  form
		
		Form<ActivityEntry> entryForm = 
				Form.form(ActivityEntry.class).bindFromRequest();
		...
		ActivityEntry entry = entryForm.get();
	...
	public  Result  readAll(Integer pageNumber,Integer sizePerPage){
		List<ActivityEntry> entryList = ActivityEntry.find.where()
				.setFirstRow(pageNumber*sizePerPage)
				.setMaxRows(sizePerPage)
				.findList();
		double total = ActivityEntry.find.where()
                .findRowCount();
